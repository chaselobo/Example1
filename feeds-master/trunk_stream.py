...
class BroadcastifyCallSystem:
    ...
    def _request(self, after_timestamp, do_init):
        ...
        print(f"Response content: {r.text[:200]}")  # Print first 200 chars
    
        # Try to handle JSON errors
        try:
            return r.json()
        except json.JSONDecodeError:
            print("ERROR: Invalid JSON response from Broadcastify API")
            print(f"Full response: {r.text}")
            # Return empty structure to avoid breaking everything
            return {"calls": [], "lastPos": self.after_timestamp}
